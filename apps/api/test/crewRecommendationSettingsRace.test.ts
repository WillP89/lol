...
describe('getOrCreateSettings: concurrent first-read race on a brand-new Crew', () => {
  ...
  test('two simultaneous callers both get the same settings row, neither throws', async () => {
    ...
    expect(a).toEqual({ enabled: true, maxPerWeek: 3, travelRadiusMeters: null, categoryPreferences: [], interestPreferences: [], preferencesSetAt: null });
    expect(b).toEqual(a);
  });
  ...
  test('a genuinely concurrent burst of 5 callers on one new Crew all succeed', async () => {
    ...
    for (const r of results) expect(r).toEqual({ enabled: true, maxPerWeek: 3, travelRadiusMeters: null, categoryPreferences: [], interestPreferences: [], preferencesSetAt: null });
  });
});
